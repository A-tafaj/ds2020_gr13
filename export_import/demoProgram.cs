        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                ...
                else if (command == "export-key")
                {
                    ...
                    string privKeysDir = dir + name + ".xml";
                    string pubKeysDir = dir + name + ".pub.xml";

                    if (File.Exists(pubKeysDir))
                    {
                        if (args.Length == 3)
                        {
                            if (type == "public")
                            {
                                string publicKey = File.ReadAllText(dir + name + ".pub.xml");
                                Console.WriteLine("\n" + publicKey + "\n");
                            }
                            else if (type == "private")
                            {
                                string privateKey = File.ReadAllText(dir + name + ".xml");
                                Console.WriteLine("\n" + privateKey + "\n");
                            }
                        }
